...
// Helper Universal Update Stok Aset (Kebal Nama Kolom Supabase)
async function updateStokAset(idBarang, selisihQty, operasi = 'kurang') {
  try {
    if (!idBarang) return;
    const { data: listAset } = await db.from('Aset').select('*');
    const safeList = makeCaseInsensitive(listAset || []);
    
    let targetAset = safeList.find(a => {
      let aId = cariNilaiKolom(a, ['id', 'id_barang', 'kode', 'barang_id']);
      return aId && aId.toString().trim() === idBarang.toString().trim();
    });

    if (targetAset) {
      let pkKey = Object.keys(targetAset).find(k => {
        let kLower = k.toLowerCase();
        return kLower === 'id' || kLower === 'id_barang' || kLower === 'kode';
      }) || 'id';

      let actualPkVal = targetAset[pkKey];
      let currentStok = parseInt(cariNilaiKolom(targetAset, ['jumlah', 'stok', 'stock']) || '0');
      let stokBaru = operasi === 'kurang' ? Math.max(0, currentStok - selisihQty) : currentStok + selisihQty;

      let stokKey = Object.keys(targetAset).find(k => {
        let kLower = k.toLowerCase();
        return kLower === 'jumlah' || kLower === 'stok' || kLower === 'stock';
      }) || 'jumlah';

      let updateObj = {};
      updateObj[stokKey] = stokBaru;

      let statusKey = Object.keys(targetAset).find(k => k.toLowerCase() === 'status');
      if (statusKey) {
        updateObj[statusKey] = stokBaru > 0 ? 'Tersedia' : 'Habis';
      }

      await db.from('Aset').update(updateObj).eq(pkKey, actualPkVal);
      console.log(`✅ Stok aset ${idBarang} berhasil diperbarui jadi ${stokBaru}`);
    }
  } catch (err) {
    console.error("Gagal update stok aset:", err);
  }
}
...
async function callGASPost(actionName, extraPayload = {}) {
  try {
    ...
    if (actionName === 'updateDataDiSheet') {
      ...
      if (sheetName.toLowerCase() === 'peminjaman' && session.role === 'RT') {
        ...
        if (newStatus.toLowerCase() === 'disetujui' && oldStatus.toLowerCase() !== 'disetujui' && idBarang) {
          await updateStokAset(idBarang, qtyAcc, 'kurang');
          ...
          if (oldStatus.toLowerCase() !== newStatus.toLowerCase() && idBarang) {
            await updateStokAset(idBarang, qtyAcc, 'tambah');
          }
        }
    ...
    // 6. Simpan Pengajuan Peminjaman Barang oleh Warga
    if (actionName === 'simpanPengajuanPeminjaman') {
      let p = extraPayload.payload || extraPayload;
      let newId = 'PMJ-' + Math.floor(1000 + Math.random() * 9000);
      ...
      const { error } = await db.from('Peminjaman').insert([insertObj]);
      if (error) return { status: 'error', message: error.message };
    ...
    if (actionName === 'verifikasiPeminjamanRT') {
      ...
      let updateData = {
        ...
        jumlah_acc: parseInt(qtyAcc) || 0,
        catatan_rt: catatanRt || '-'
      };

      const { error } = await db.from('Peminjaman').update(updateData).eq('id', idPinjam);
      if (error) return { status: 'error', message: error.message };
      ...
      if (status === 'Disetujui') {
        ...
        if (safePinjam) {
          let idBarang = cariNilaiKolom(safePinjam, ['id_barang', 'idbarang', 'barang_id']);
          if (idBarang) {
            await updateStokAset(idBarang, parseInt(qtyAcc), 'kurang');
          }
        }
      }
    ...
    if (actionName === 'prosesPengembalianAsetRT') {
      ...
      let updateData = {
        ...
        catatan_rt: `Dikembalikan ${qtyKembali} unit. ` + (catatanRt || '')
      };
      ...
      if (safePinjam) {
        let idBarang = cariNilaiKolom(safePinjam, ['id_barang', 'idbarang', 'barang_id']);
        if (idBarang) {
          await updateStokAset(idBarang, parseInt(qtyKembali || 0), 'tambah');
        }
      }

      return { status: 'success', message: 'Pengembalian barang berhasil dicatat & stok dikembalikan!' };
    }
...
async function callGASGet(actionName, params = {}) {
  try {
    ...
    // 7. Get Daftar Barang Aset untuk Dropdown Peminjaman Warga
    if (actionName === 'getDaftarBarangAset') {
      const { data, error } = await db.from('Aset').select('*');
      const safeAset = makeCaseInsensitive(data);
      if (error || !safeAset) return { status: 'success', data: [] };

      let listBarang = safeAset.map(item => ({
        id: cariNilaiKolom(item, ['id', 'id_barang', 'kode']) || item.id || '',
    ...
    // 8. Get Riwayat Peminjaman Warga & RT
    if (actionName === 'getRiwayatPeminjaman') {
      const { data, error } = await db.from('Peminjaman').select('*');
      const safeData = makeCaseInsensitive(data);
      if (error || !safeData) return { status: 'success', data: [] };

      let cleanRole = (session.role || 'warga').toLowerCase();
      let filtered = safeData;
      if (cleanRole !== 'rt') {
      ...
      let riwayat = filtered.map(item => ({
        ...
        idBarang: cariNilaiKolom(item, ['id_barang', 'idbarang', 'barang_id']) || '',
        ...
        keterangan: cariNilaiKolom(item, ['keterangan', 'ket']) || '-',
        catatanRt: cariNilaiKolom(item, ['catatan_rt', 'catatanrt', 'lokasi']) || '-',
...
function generateFormInputs(rowData) {
  ...
  currentHeaders.forEach((h, idx) => {
    if(['id', 'no', 'saldo'].includes(h.toLowerCase()) && currentActiveMenu !== 'Aset') return;
